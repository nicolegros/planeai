...
| `--priority`   | If the user signals urgency ("urgent", "critical", "high priority", "before the release"). Higher number = higher priority. Only set if there's a real signal; default (0) is fine.                                                                                        |
