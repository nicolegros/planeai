...
#[test]
fn migrates_legacy_jira_settings_to_plugin_storage_once() {
    let config_dir = tempfile::tempdir().unwrap();
    let app_data_dir = tempfile::tempdir().unwrap();
    let mut config = Config::default();
    config.integrations = Some(IntegrationsConfig {
        jira: Some(planeai_jira::config::JiraConfig {
            site: "https://legacy.atlassian.net".to_string(),
            sync_interval_ms: 120_000,
            sources: HashMap::from([(
                "legacy".to_string(),
                planeai_jira::config::JiraSyncSource {
    ...
    save(config_dir.path(), &config).unwrap();

    let settings_dir = app_data_dir.path().join("plugins/state/jira/data");
    fs::create_dir_all(&settings_dir).unwrap();
    fs::write(
        settings_dir.join("settings.json"),
        r#"{"site":"https://plugin.atlassian.net","sources":{"plugin":{"jql":"project = PLUGIN"}}}"#,
    )
    .unwrap();

    assert!(migrate_legacy_jira_plugin_settings(
        config_dir.path(),
        app_data_dir.path(),
        &mut config
    )
    ...
    assert_eq!(settings["site"], "https://plugin.atlassian.net");
    assert_eq!(settings["sync_interval_ms"], 120_000);
    assert_eq!(settings["sources"]["plugin"]["jql"], "project = PLUGIN");
    assert_eq!(settings["sources"]["legacy"]["jql"], "project = LEGACY");
    assert!(!fs::read_to_string(config_dir.path().join("config.json"))
        .unwrap()
