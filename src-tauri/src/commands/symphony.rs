...
#[tauri::command]
pub fn get_symphony_status(state: State<SymphonyHandle>, db_state: State<crate::DbState>, config_state: State<crate::ConfigState>) -> Result<String, String> {
    ...
    let max_concurrent: usize = cfg.task_managers.values()
        .filter_map(|tm| tm.auto_dispatch.as_ref())
        .map(|ad| ad.max_concurrent)
        .next()
        .unwrap_or(3);
    Ok(format!("{{\"active\":true,\"slots_used\":{slots_used},\"max_concurrent\":{max_concurrent}}}"))
}
