...
fn install_cli(app: tauri::AppHandle) -> Result<(), String> {
    ...
    let cli_bin = exe_dir.parent().unwrap_or(&exe_dir).join("MacOS").join("planeai-cli");
    if !cli_bin.exists() {
        ...
        if !alt.exists() {
            return Err(format!("CLI binary not found at {:?} or {:?}", cli_bin, alt));
        }
...
fn symlink_cli(source: &std::path::Path) -> Result<(), String> {
    ...
    if target.exists() || target.symlink_metadata().is_ok() {
        std::fs::remove_file(target).map_err(|e| format!("failed to remove existing symlink: {e}"))?;
    }
