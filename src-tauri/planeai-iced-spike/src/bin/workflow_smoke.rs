...
fn main() {
    ...
    let r = step!(
        ...
        Ok(()).and_then(|_| ensure_daemon_running_sync())
    );
    if r.is_err() {
        all_pass = false;
    }
    ...
    match &r {
        Ok(data) => {
            metrics.insert("first_output_bytes".into(), (data.len() as u64).into());
        }
        Err(_) => {
            all_pass = false;
        }
    ...
    if let Ok(log_dir) = std::env::var("PLANEAI_SESSION_LOG_DIR") {
        ...
        eprintln!(
            "  [{:>34}] SKIP (PLANEAI_SESSION_LOG_DIR not set)",
            "durable_log_exists"
        );
        eprintln!(
            "  [{:>34}] SKIP (PLANEAI_SESSION_LOG_DIR not set)",
    ...
    metrics.insert(
        "result".into(),
        if all_pass { "pass" } else { "fail" }.into(),
    );
