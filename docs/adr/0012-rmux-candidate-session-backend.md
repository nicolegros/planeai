...
- A TaskWorkspace owns exactly one PlaneAI-owned rmux session, created when the workspace is first materialised and destroyed **only when its task is explicitly deleted**. Moving a task to `Done` archives its agent panes but retains the workspace and its rmux session.
