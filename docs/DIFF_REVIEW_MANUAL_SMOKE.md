...
8. Start a non-empty comment draft. Select another range in the same file and verify the draft remains intact. Then try switching files, refreshing, changing split/unified mode, changing the comparison, or opening a different annotation/comment-list entry: each must request confirmation before discarding the draft. Cancel leaves the draft; confirm discards it.
9. Verify selections clear after submitting or canceling a comment, pressing Escape, refreshing, selecting a different file, clicking a different line, or clicking blank diff space.
