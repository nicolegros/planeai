<script lang="ts">
  ...
  import { GitPullRequest, GitMerge, RefreshCw, Bot, Terminal, GitCompare, FileCode, ShieldAlert } from "@lucide/svelte";
  ...
  let mergeStateFetchedFor = $state<string | null>(null);
  ...
  async function fetchMergeState() {
    if (!sessionId || mergeStateFetchedFor === sessionId) return;
    try {
      ...
      mergeStateFetchedFor = sessionId;
    } catch {
      mergeBlocked = false;
      mergeBlockReasons = [];
  ...
  async function doMerge(strategy: string) {
    ...
    try {
      ...
      const msg = String(e);
      if (msg.startsWith("MERGE_BLOCKED:")) {
        mergeBlocked = true;
        mergeBlockReasons = msg.slice("MERGE_BLOCKED:".length).split("|");
        // Refresh settings URL
        mergeStateFetchedFor = null;
        fetchMergeState();
        showSnackbar("Merge blocked by branch protection", "error");
      } else {
  ...
  $effect(() => { if (prUrl && sessionId && !isMerged) { fetchStrategies(); fetchMergeState(); } });
        ...
        {#if prPanelOpen}
          ...
          <div class="absolute top-[31px] right-0 z-[70] w-[282px] rounded-xl border border-border-s bg-panel shadow-[0_20px_54px_-12px_rgba(0,0,0,0.55)] overflow-hidden">
            ...
            {#if mergeBlocked && !isMerged}
              <div class="px-[13px] py-[11px] border-b border-border bg-status-exited/8">
