...
<Dialog.Root {open} onOpenChange={(v) => { if (!v) close(); else onOpenChange(v); }}>
  <Dialog.Portal>
    ...
    <Dialog.Content
      class="fixed left-1/2 top-[88px] z-50 w-[600px] -translate-x-1/2 rounded-xl border border-border-strong bg-surface-50 dark:bg-surface-700 shadow-[0_26px_70px_-16px_rgba(0,0,0,0.5)] overflow-hidden"
    >
      ...
      {#if subMenu === "archivedSessions"}
        <Command.Root class="flex flex-col" loop disablePointerSelection>
          <Command.Input
            class="h-[54px] w-full border-b border-border bg-transparent px-4 text-[13.5px] outline-none placeholder:text-text-3"
            placeholder="Search archived sessions..."
          />
          <Command.List class="max-h-72 overflow-y-auto p-2">
            <Command.Viewport>
              <Command.Empty class="flex items-center justify-center py-6 text-[13px] text-text-3">
                No archived sessions.
              </Command.Empty>
              <Command.Group>
              <Command.GroupHeading class="px-3 pb-1 pt-3 text-[10px] font-semibold text-text-3 uppercase tracking-[.05em]">Archived Sessions</Command.GroupHeading>
                <Command.GroupItems>
                  {#each archivedSessions as session (session.id)}
                    <Command.Item
                      ...
                      class="flex h-9 cursor-pointer items-center justify-between rounded-lg px-3 text-[13px] text-text-1 data-selected:bg-primary-500/15"
                      onSelect={() => { onRestoreSession(session.id); archivedSessions = archivedSessions.filter(s => s.id !== session.id); }}
                    >
                      <span class="truncate">{session.name || session.branch} <span class="text-xs text-text-3">({projectName(session.project_id)})</span></span>
                      <span class="text-xs text-primary-500 shrink-0 ml-2">Restore</span>
                    </Command.Item>
                  {/each}
                </Command.GroupItems>
              </Command.Group>
              ...
              <Command.Group>
                <Command.GroupHeading class="px-3 pb-1 pt-3 text-[10px] font-semibold text-text-3 uppercase tracking-[.05em]">Delete Archived</Command.GroupHeading>
                <Command.GroupItems>
                  {#each archivedSessions as session (session.id)}
                    <Command.Item
                      ...
                      class="flex h-9 cursor-pointer items-center justify-between rounded-lg px-3 text-[13px] text-error-400 data-selected:bg-primary-500/15"
      ...
      {:else if subMenu === "archiveProject"}
        <Command.Root class="flex flex-col" loop disablePointerSelection>
          ...
          <Command.List class="max-h-72 overflow-y-auto p-2">
            <Command.Viewport>
              <Command.Empty class="flex items-center justify-center py-6 text-[13px] text-text-3">No projects.</Command.Empty>
              <Command.Group>
                <Command.GroupItems>
                  {#each projects as project (project.id)}
                    <Command.Item
                      value="archive {project.name}"
                      class="flex h-9 cursor-pointer items-center rounded-lg px-3 text-[13px] text-text-1 data-selected:bg-primary-500/15"
                ...
                <Command.GroupItems>
                  {#each projects as project (project.id)}
                    <Command.Item
                      ...
                      class="flex h-9 cursor-pointer items-center rounded-lg px-3 text-[13px] text-error-400 data-selected:bg-primary-500/15"
      ...
      {:else if subMenu === "restoreProject"}
        <Command.Root class="flex flex-col" loop disablePointerSelection>
          ...
          <Command.List class="max-h-72 overflow-y-auto p-2">
            <Command.Viewport>
              <Command.Empty class="flex items-center justify-center py-6 text-[13px] text-text-3">No archived projects.</Command.Empty>
              <Command.Group>
                <Command.GroupItems>
      ...
      {:else if subMenu === "pickTask"}
        <Command.Root class="flex flex-col" loop disablePointerSelection>
          ...
          <Command.List class="max-h-72 overflow-y-auto p-2">
            <Command.Viewport>
              <Command.Empty class="flex items-center justify-center py-6 text-[13px] text-text-3">No tasks found.</Command.Empty>
              <Command.Group>
                <Command.GroupItems>
                  {#each taskItems as task (task.key)}
                    ...
                    >
                      <span class="font-medium font-mono text-primary-500 mr-2">{task.key}</span>
      ...
      {:else if subMenu === "openFile"}
        <Command.Root class="flex flex-col" loop disablePointerSelection>
          ...
          <Command.List class="max-h-72 overflow-y-auto p-2">
            <Command.Viewport>
              <Command.Empty class="flex items-center justify-center py-6 text-[13px] text-text-3">No files found.</Command.Empty>
              <Command.Group>
                <Command.GroupItems>
                  {#each fileList as file (file)}
                    <Command.Item
                      value={file}
                      class="flex h-9 cursor-pointer items-center rounded-lg px-3 text-[12.5px] font-mono text-text-1 data-selected:bg-primary-500/15"
              ...
              <Command.Group>
                <Command.GroupItems>
                  {#each projects as project (project.id)}
                    ...
                    >
                      <span>{project.name}</span>
                      <span class="text-xs shrink-0 ml-2 {projectAutoModes[project.id] ? 'text-status-running' : 'text-text-3'}">{projectAutoModes[project.id] ? '✓ On' : 'Off'}</span>
                    </Command.Item>
                  {/each}
                </Command.GroupItems>
              </Command.Group>
      ...
      <Command.Root class="flex flex-col" loop disablePointerSelection>
        ...
        <Command.List class="max-h-72 overflow-y-auto p-2">
          <Command.Viewport>
            ...
            <Command.Group>
              <Command.GroupHeading class="px-3 pb-1 pt-3 text-[10px] font-semibold text-text-3 uppercase tracking-[.05em]">Sessions</Command.GroupHeading>
              <Command.GroupItems>
                {#each sessions as session (session.id)}
                  <Command.Item
                    value="session {session.name || session.branch} {session.id}"
                    keywords={[session.name, session.branch]}
                    class="flex h-9 cursor-pointer items-center gap-2 rounded-lg px-3 text-[13px] text-text-1 data-selected:bg-primary-500/15 {session.id === activeSessionId ? 'font-medium' : ''}"
            ...
            <Command.Group>
              <Command.GroupHeading class="px-3 pb-1 pt-3 text-[10px] font-semibold text-text-3 uppercase tracking-[.05em]">Actions</Command.GroupHeading>
              <Command.GroupItems>
                <Command.Item
                  value="copy project root path"
                  keywords={["copy", "path", "root", "directory", "cwd"]}
                  disabled={!activeSessionId}
                  class="flex h-9 cursor-pointer items-center gap-2 rounded-lg px-3 text-[13px] text-text-1 data-selected:bg-primary-500/15 aria-disabled:opacity-50 aria-disabled:cursor-not-allowed"
                ...
                <Command.Item
                  ...
                  disabled={!activeSessionId}
                  class="flex h-9 cursor-pointer items-center gap-2 rounded-lg px-3 text-[13px] text-error-400 data-selected:bg-primary-500/15 aria-disabled:opacity-50 aria-disabled:cursor-not-allowed"
                ...
                <Command.Item
                  ...
                  class="flex h-9 cursor-pointer items-center gap-2 rounded-lg px-3 text-[13px] text-text-1 data-selected:bg-primary-500/15"
                ...
                >
                  Review Changes
                  <kbd class="ml-auto text-[10px] font-mono text-text-3">⌘⇧R</kbd>
                </Command.Item>
                <Command.Item
                  value="pull request"
      ...
      {/if}
      <!-- Footer hints -->
      <div class="h-[42px] flex items-center gap-4 px-4 border-t border-border bg-surface-200 dark:bg-surface-600 text-[11px] text-text-3">
