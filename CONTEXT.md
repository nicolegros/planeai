...
- Archive/delete/restart of one agent session closes only its own windows, never the shared session or a sibling agent's panes. Shell tabs are windows too, so they persist with the workspace instead of dying with the app.
