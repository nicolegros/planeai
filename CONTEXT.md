...
| **Jira integration** | Optional two-way sync between a Jira Cloud site and planeai's task board. Configured via `integrations.jira` in the config file. Syncs issues matching JQL filters into local tasks, and writes back status transitions (start/complete) and comments to Jira. OAuth 2.0 (PKCE) for auth.                                                                                               |
...
- **planeai-jira** crate for Jira Cloud integration (OAuth 2.0 PKCE auth, issue sync, status writeback)
