...
| **Jira integration** | Optional Jira Cloud connection managed by the bundled Jira plugin. The plugin stores its site and sync interval under its own settings namespace and OAuth credentials under its backend-only secrets namespace. Source sync, task writeback, and issue-sidebar workflows are deferred pending the later parity slice. OAuth 2.0 (PKCE) is used for auth. |
