...
| **Jira integration** | Optional Jira Cloud connection managed by the bundled Jira plugin. The plugin stores its site and future sync interval under its own settings namespace and OAuth credentials under its backend-only secrets namespace. OAuth 2.0 (PKCE) is used for authorization. Source sync, task writeback, issue-sidebar workflows, and departed-issue handling are deferred to a later parity slice.                                                                                                                                        |
...
- **planeai-plugin-jira** executable for Jira Cloud connection ownership (OAuth 2.0 PKCE, plugin-scoped settings and backend-only secrets); issue sync and writeback remain deferred
